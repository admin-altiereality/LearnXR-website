...
router.post('/determine-provider', (req, res) => {
    try {
        ...
        const provider = (0, subscription_1.shouldUseRazorpay)(effectiveCountry) ? 'razorpay' : 'paddle';
        return res.json({
            success: true,
            data: {
                provider,
...
router.post('/create-order', async (req, res) => {
    try {
        ...
        if ((0, firebase_admin_1.isFirebaseInitialized)() && firebase_admin_1.db) {
            await firebase_admin_1.db.collection('orders').doc(order.id).set({
                ...order,
                userId,
                planId,
                billingCycle: billingCycle || 'monthly',
    ...
    try {
        ...
        if (generated_signature === razorpay_signature) {
            ...
            if (userId && planId && (0, firebase_admin_1.isFirebaseInitialized)() && firebase_admin_1.db) {
                ...
                await (0, subscriptionService_1.upsertSubscription)(userId, {
                    planId,
                ...
                // Update order status
                await firebase_admin_1.db.collection('orders').doc(razorpay_order_id).update({
                    status: 'paid',
                    paymentId: razorpay_payment_id,
 ...
 */
router.post('/razorpay/webhook', express_1.default.raw({ type: 'application/json' }), async (req, res) => {
    try {
        ...
        // Map event to status
        const newStatus = (0, subscription_1.mapRazorpayStatus)(eventType);
        if (newStatus) {
            const webhookEvent = {
                id: eventId,
                type: eventType,
            ...
            if (eventType === 'subscription.charged') {
                await (0, subscriptionService_1.resetUsageForNewPeriod)(userId);
            }
            const processed = await (0, subscriptionService_1.updateSubscriptionFromWebhook)(userId, eventId, updates, webhookEvent);
 ...
 */
router.post('/paddle/webhook', express_1.default.raw({ type: 'application/json' }), async (req, res) => {
    try {
        ...
        // Map event to status
        const newStatus = (0, subscription_1.mapPaddleStatus)(eventType);
        if (newStatus) {
            const webhookEvent = {
                id: eventId,
                type: eventType,
 ...
 * Map Paddle product ID to internal plan ID
 */
function mapPaddleProductToPlan(productId) {
    const mapping = {
