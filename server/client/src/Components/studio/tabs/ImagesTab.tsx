...
const imageTypes = [
  { value: 'diagram', label: 'Diagram', color: 'text-cyan-400 bg-cyan-500/10 border-cyan-500/20' },
...
export const ImagesTab = ({ chapterId, topicId, bundle }: ImagesTabProps) => {
  ...
  const handleDeleteImage = async () => {
    ...
    try {
      ...
      if (imageToDelete.image_url?.includes('firebase')) {
        try {
          const storageRef = ref(storage, imageToDelete.image_url);
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-400">Loading images from chapter_images...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="h-full flex flex-col bg-[#0a0f1a]">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center bg-slate-800/50 rounded-lg border border-slate-700/50 p-0.5">
            <button
              ...
              className={`p-2 rounded-md transition-all ${viewMode === 'grid' ? 'bg-violet-500/20 text-violet-400' : 'text-slate-400 hover:text-white'}`}
            >
              <Grid3X3 className="w-4 h-4" />
            </button>
            <button
              onClick={() => setViewMode('list')}
              className={`p-2 rounded-md transition-all ${viewMode === 'list' ? 'bg-violet-500/20 text-violet-400' : 'text-slate-400 hover:text-white'}`}
            >
      ...
      {images.length === 0 ? (
        ...
        <div className="flex-1 flex overflow-hidden">
          {/* Image Grid/List */}
          <div className={`${selectedImage ? 'w-1/2' : 'flex-1'} border-r border-slate-700/50 overflow-y-auto p-4 bg-slate-900/30 transition-all`}>
            {viewMode === 'grid' ? (
              <div className="grid grid-cols-2 xl:grid-cols-3 gap-3">
                {images.map((image) => {
                  ...
                  return (
                    ...
                    >
                      {/* Image */}
                      <div className="aspect-video bg-slate-900/50 flex items-center justify-center overflow-hidden relative">
                        <img
                          src={image.image_url}
                          alt={image.name}
              ...
              <div className="space-y-2">
                {images.map((image) => {
                  ...
                  return (
                    ...
                    >
                      {/* Thumbnail */}
                      <div className="w-20 h-14 bg-slate-900/50 rounded-lg flex items-center justify-center overflow-hidden flex-shrink-0">
                        <img
          ...
          {selectedImage && (
            <div className="flex-1 flex flex-col">
              <div className="flex-1 relative bg-slate-900/50 flex items-center justify-center p-4">
                ...
                <div className="absolute top-4 right-4 flex items-center gap-2">
                  <button
                    ...
                    className="p-2.5 bg-black/60 hover:bg-black/80 rounded-lg backdrop-blur-sm border border-white/10 text-white/80"
                  >
                    <Maximize2 className="w-5 h-5" />
                  </button>
                  <a
              ...
              <div className="p-5 border-t border-slate-700/50 bg-slate-800/30">
                ...
                <div className="grid grid-cols-2 gap-4 p-4 bg-slate-900/30 rounded-xl border border-slate-700/30">
                  <div>
                    <p className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Image ID</p>
                    <p className="text-xs text-slate-300 font-mono truncate">{selectedImage.id}</p>
                  </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <select
                            ...
                            className="text-xs bg-slate-900/50 border border-slate-600/50 rounded px-2 py-0.5 text-slate-300"
                          >
                            {imageTypes.map(type => (
      ...
      {showLightbox && selectedImage && (
        ...
        >
          <button
            className="absolute top-4 right-4 p-3 text-white/60 hover:text-white"
            onClick={() => setShowLightbox(false)}
          >
            <X className="w-8 h-8" />
          </button>
