...
export interface FinalizeGeneratedAssetRequest {
  sourceAssetId: string;
  sourceCollection: 'text_to_3d_assets' | 'avatar_to_3d_assets' | 'meshy_assets';
  chapterId: string;
  topicId: string;
  userId?: string;
  prompt?: string;
...
export interface FinalizeAnimatedAssetRequest {
  assetKind: 'animation';
  sourceAssetId: string;
  sourceCollection: 'text_to_3d_assets' | 'avatar_to_3d_assets' | 'meshy_assets';
  meshyAssetId?: string;
...
export interface RegisterUploadedAssetRequest {
  assetKind: 'uploaded';
  assetId: string;
  storagePath: string;
  chapterId: string;
...
export class MeshyApiService {
  ...
  private async makeProxyRequest(
    ...
    const firebaseAuthHeader = currentUser
  ...
  /**
   * Store generated Meshy model files in Firebase Storage and register render URLs.
   */
  async finalizeGeneratedAsset(params: FinalizeGeneratedAssetRequest): Promise<PersistedAssetResponse> {
    const user = auth.currentUser;
    if (!user) throw new Error('Authentication required to save generated 3D assets');
    const token = await user.getIdToken();
    const res = await fetch(`${this.proxyBaseUrl}/meshy/finalize-generated-asset`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify(params),
    ...
    const raw = data?.data ?? data;
    if (!res.ok) {
      throw new Error(data?.message || raw?.message || raw?.error || `Failed to save generated asset: ${res.status}`);
    }
    return raw as PersistedAssetResponse;
  }

  /**
   * Store an animated GLB generated by Meshy and update the source asset records.
   */
  async finalizeAnimatedAsset(params: FinalizeAnimatedAssetRequest): Promise<PersistedAssetResponse> {
    const user = auth.currentUser;
    if (!user) throw new Error('Authentication required to save animated 3D assets');
    const token = await user.getIdToken();
    const res = await fetch(`${this.proxyBaseUrl}/meshy/finalize-generated-asset`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
    ...
    const raw = data?.data ?? data;
    if (!res.ok) {
      throw new Error(data?.message || raw?.message || raw?.error || `Failed to save animated asset: ${res.status}`);
    }
    return raw as PersistedAssetResponse;
  }

  /**
   * Register a manually uploaded GLB and receive its tokenized render URL.
   */
  async registerUploadedAsset(params: RegisterUploadedAssetRequest): Promise<PersistedAssetResponse> {
    const user = auth.currentUser;
    if (!user) throw new Error('Authentication required to register uploaded 3D assets');
    const token = await user.getIdToken();
    const res = await fetch(`${this.proxyBaseUrl}/meshy/finalize-generated-asset`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
    ...
    const raw = data?.data ?? data;
    if (!res.ok) {
      throw new Error(data?.message || raw?.message || raw?.error || `Failed to register uploaded asset: ${res.status}`);
    }
    return raw as PersistedAssetResponse;
  }
