...
const N8N_PARTNER_WEBHOOK_URL = process.env.N8N_PARTNER_WEBHOOK_URL;
...
const computeLeadScore = (data: {
  ...
  const tier = leadScore >= 70 ? 'A' : leadScore >= 45 ? 'B' : 'C';
  return { leadScore, tier };
};

const handlePartnerRegister = async (req: express.Request, res: express.Response) => {
  ...
  if (!consent) {
    return res.status(400).json({ success: false, message: 'Consent is required to submit.' });
  }
  ...
  try {
    ...
    console.error('Failed to store partner registration:', error);
  ...
  if (webhookUrl) {
    try {
      ...
      console.error('Partner webhook forward failed (non-fatal):', error);
    }
  }
