/**
 * Authenticated Twilio inbox API — mount under /api/twilio-inbox with verifyFirebaseToken + requireInboxStaff
 */

import express, { Request, Response } from 'express';
import twilio from 'twilio';
import { verifyFirebaseToken } from '../middleware/authMiddleware';
import { requireInboxStaff } from '../middleware/inboxStaffMiddleware';
...
router.use(requireInboxStaff);

function normalizeTwilioCredential(value: string): string {
...
function twilioConfig() {
  const accountSid = normalizeTwilioCredential(process.env.TWILIO_ACCOUNT_SID || '');
  const authToken = normalizeTwilioCredential(process.env.TWILIO_AUTH_TOKEN || '');
  const apiKeySid = normalizeTwilioCredential(process.env.TWILIO_API_KEY_SID || '');
  const apiKeySecret = normalizeTwilioCredential(process.env.TWILIO_API_KEY_SECRET || '');
  const conversationsServiceSid = normalizeTwilioCredential(
    process.env.TWILIO_CONVERSATIONS_SERVICE_SID || '',
  );
  const whatsappFrom = (process.env.TWILIO_WHATSAPP_FROM || '').trim();
  const messagingServiceSid = (process.env.TWILIO_MESSAGING_SERVICE_SID || '').trim();
  const accessTokenRegion = normalizeTwilioCredential(process.env.TWILIO_ACCESS_TOKEN_REGION || '');
  return {
    accountSid,
    authToken,
    apiKeySid,
...
/** REST list plus Firestore `whatsappInboxMeta.conversationSid` lookups (covers list gaps / race). */
    ...
    const emptyHint =
      items.length === 0
        ? [
            ...
            'WhatsApp Sandbox (+1415…): clear the Sandbox inbound webhook (Messaging > Try it out > WhatsApp > Sandbox settings) — see https://www.twilio.com/docs/conversations/use-twilio-sandbox-for-whatsapp',
            'Production WhatsApp: the Messaging Service that contains your WhatsApp sender must match Conversations > Manage > Defaults > Default Messaging Service.',
            'After fixing Twilio, send a new inbound WhatsApp message and refresh.',
          ].join(' ')
        : null;
