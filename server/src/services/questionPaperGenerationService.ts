 ...
 * using OpenAI in JSON mode. Paper structure follows the Delhi DoE / CBSE pattern
 * with sections (A/B/C/D), internal choices, and optional answer key.
 */

import OpenAI from 'openai';
import { storage as adminStorage } from '../config/firebase-admin';
...
async function extractSourceText(
  ...
  try {
    // pdf-parse has no types in some versions; import lazily.
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const pdfParse = (await import('pdf-parse')).default ?? (await import('pdf-parse'));
    const parsed = await (pdfParse as (b: Buffer) => Promise<{ text: string }>)(buffer);
...
3. Use the Indian school English conventions (e.g. "Tick the correct answer", "Encircle", "Fill in the blanks") and Indian contextual examples (rupees, local names, local scenarios).
...
export async function generateQuestionPaper(input: GenerateInput): Promise<GenerateOutput> {
  ...
  try {
    ...
    // On rate-limit or 5xx, try the fallback model once.
...
function normalizePaper(
  ...
  // Map section names in blueprint to section_ids for fallback matching
  ...
  const questions: GeneratedQuestion[] = rawQs.map((q: unknown, idx: number) => {
    ...
    const pairsRaw = Array.isArray(r.pairs)
      ? (r.pairs as Array<Record<string, unknown>>).map((p) => ({ left: String(p.left ?? ''), right: String(p.right ?? '') }))
      : undefined;
    ...
    return {
      ...
      difficulty: ['easy', 'medium', 'hard'].includes(String(r.difficulty)) ? (r.difficulty as Difficulty) : undefined,
