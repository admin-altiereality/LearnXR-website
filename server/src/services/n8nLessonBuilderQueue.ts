...
import { db, storage, isFirebaseInitialized } from '../config/firebase-admin';
...
function getDb() {
  if (!db || !isFirebaseInitialized()) {
    throw new Error('Firestore is not configured. Add Firebase Admin credentials for local API.');
  }
  return db;
}
...
function getBucket() {
  if (!storage || !isFirebaseInitialized()) {
    throw new Error('Firebase Storage is not configured. Add Firebase Admin credentials for local API.');
  }
